...
function listafiltroprod(){
		inicializar_XHR();//inicsializa el HTTPrequest
		var filtro= document.getElementById("filtro").value;
		parametros = "filtro=" + filtro;
		if(filtro==""){
			//alert('Vacio');
			peticionHTTP.open("POST", "llamarlistaprod.php", true);//ubica el archivo y manda datos con el metodo POST asincrono
		...
		peticionHTTP.open("POST", "listaprodfiltro.php", true);//ubica el archivo y manda datos con el metodo POST asincrono
		peticionHTTP.onreadystatechange = function (){
					if(peticionHTTP.readyState == 4)// valor numerico del estado de la peticion
						if(peticionHTTP.status == 200){// valor numerico de una respuesta correcta
							//alert(peticionHTTP.responseText);
